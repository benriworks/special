...
const STRINGS: Record<string, { ja: string; en: string }> = {
  ...
  aboutIntro: {
    ja: '流体、銀河、群れ、模様、そして文字——5つのGPUシミュレーションが、あなたのデバイスの中だけでリアルタイムに光ります。サーバーなし、アップロードなし、追跡なし。ただ触れて、光をかき混ぜてください。',
    en: 'Fluid, galaxy, flock, pattern, letters — five GPU simulations glowing in real time, entirely on your device. No servers, no uploads, no tracking. Just reach in and stir the light.',
  },
  aboutMadeByTitle: { ja: 'AIチームがつくりました', en: 'Built by a team of AIs' },
  aboutMadeByBody: {
    ja: 'デザイン会議が世界観を描き、基盤エンジニアがエンジンを組み、5体のエージェントがそれぞれのモードを並行して実装。意地悪なレビュアーが粗を探し、自動検証ハーネスが全モードに緑のランプを灯すまで作り直す——その全体を、Claudeが指揮しました。',
    en: 'A design panel sketched the vision, a foundation engineer built the engine, and five agents grew one mode each in parallel. Adversarial reviewers hunted for flaws, and nothing shipped until an automated verify harness turned every mode green — the whole ensemble orchestrated by Claude.',
  },
  ...
  creditGraphicsBy: { ja: 'モード担当エージェント ×5', en: 'Five mode engineer agents' },
